            ...
            fields.forEach(function(field) {
                ...
                if (isRequired && value === '') {
                    ...
                    if (!emailRegex.test(value)) {
                        errorMsg = field.getAttribute('data-error-email') || 'Please enter a valid email address.';
                    }
                } else if (fieldType === 'tel' && value !== '') {
                    const telRegex = /^[\d\s\-\+\(\)]{7,}$/;
                    if (!telRegex.test(value)) {
                        errorMsg = field.getAttribute('data-error-tel') || 'Please enter a valid phone number.';
                    }
                }
                if (errorMsg) {
                    isValid = false;
            ...
            if (isValid) {
                ...
                fetch(action, options)
                    .then(function(response) {
                        if (response.ok) {
                            const successMsg = contactForm.querySelector('.form-success') || document.createElement('div');
                            if (!contactForm.querySelector('.form-success')) {
                    ...
                    .catch(function() {
                        const errorMsg = contactForm.querySelector('.form-global-error') || document.createElement('div');
                        if (!contactForm.querySelector('.form-global-error')) {
                            errorMsg.className = 'form-global-error';
                            errorMsg.textContent = contactForm.getAttribute('data-error-message') || 'Something went wrong. Please try again.';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'ffa6400d-8948-4f80-b27f-834f0cf4d269',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'ffa6400d-8948-4f80-b27f-834f0cf4d269';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ffa6400d-8948-4f80-b27f-834f0cf4d269';
